...
def inception_v3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('/home/agoel/object_detection/inception_v3_google-1a9a5a14.pth'))
        return model

    return Inception3(**kwargs)


class Inception3Conv(nn.Module):
    def __init__(self, original_model, num_classes=1000, aux_logits=True, transform_input = False):
        super(Inception3Conv, self).__init__()
        ...
        if aux_logits:
            self.AuxLogits = original_model.AuxLogits
...
class Inception3(nn.Module):
    ...
    def forward(self, x, onlyConv=False):
        if self.transform_input:
            x = x.clone()
            x[:, 0] = x[:, 0] * (0.229 / 0.5) + (0.485 - 0.5) / 0.5
            x[:, 1] = x[:, 1] * (0.224 / 0.5) + (0.456 - 0.5) / 0.5
        ...
        #x = self.fc(x)
        # 1000 (num_classes)
        #if self.training and self.aux_logits and onlyConv:
        #    return x, aux, conv_output
        return x
